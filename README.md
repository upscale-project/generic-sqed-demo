...
is available here (will be installed by running script `setup-sqed-generator.sh`, see below):
...
- Run the script `./setup-sqed-generator.sh` to install the SQED
  generator (a git repository will be cloned from GitHub).
...
- Run the script `./generate-qed-files.sh` to generate the Verilog
  ...
  - In this step, several Python scripts located in subdirectory
    `./sqed-generator/SQED-Generator/` are executed to produce the
    sources of the QED module from the ISA/design specification file
    `sqed-generator/SQED-Generator/FormatFiles/RV32M-ridecore_format.txt`. The
    ...
    `sqed-generator/SQED-Generator/QEDFiles`.
